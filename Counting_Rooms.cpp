...
int n, m;
void dfs(vector<string>& v, vvi& visited, int i, int j){
  if (visited[i][j]) return;
  if (v[i][j] != '.') return;
  visited[i][j] = 1;
  if (i > 0) dfs(v, visited, i-1, j);
  if (j > 0) dfs(v, visited, i, j-1);
  if (i < n - 1) dfs(v, visited, i+1, j);
  if (j < m - 1) dfs(v, visited, i, j+1);
}
