...
pair<bool, vector<vector<pair<int, int>>>> dijkstra(int sx,int sy, int n, int m, int ex, int ey, vector<string>& in){
  auto visited = vec(n, m, false);
  auto distances = vec(n, m, INT_MAX - 1);
  auto parent = vec(n, m, pair(-1, -1));

  priority_queue <pair<int, pair<int, int>>, vector<pair<int, pair<int, int>>>, greater<pair<int, pair<int, int>>>> q;
  distances[sx][sy] = 0;
  parent[sx][sy] = pair(sx,sy);

  q.push(pair(0, pair(sx, sy)));

  vpi poss = {pair(-1, 0), pair(1, 0), pair(0, -1), pair(0, 1)};
  ...
  while(!q.empty()){
    ...
    fora(poss, x){
      ...
      else if (in[xx][xy] == '#') continue;
      else{
        if (distances[xx][xy] > (distances[ux][uy] + 1)){
          distances[xx][xy] = distances[ux][uy] + 1;
          q.push(pair(distances[xx][xy], pair(xx ,xy)));
          parent[xx][xy] = pair(ux, uy);
        }
      }
    }
  ...
  // print(distances);
  return pair(distances[ex][ey] != (INT_MAX - 1) ,parent);
}
...
void solve(){
  ...
  pair<bool, vector<vector<pair<int, int>>>> _ = dijkstra(sx, sy, n, m, ex, ey, v);
  bool ispos = _.first;
  vector<vector<pair<int, int>>> parent = _.second;
  ...
  while(!(tx == parent[tx][ty].first && ty == parent[tx][ty].second)){
    s.push(pair(tx, ty));
    pii p =  parent[tx][ty];
    tx = p.first;
    ty = p.second;
  ...
  while(!s.empty()){
    pii p = s.top();
    s.pop();
    if (p.first < tx) out.pb('U');
    else if (p.first > tx) out.pb('D');
    else if (p.second < ty) out.pb('L');
    else if (p.second > ty) out.pb('R');
