...
void push(int v, int low, int high){
  if (laz[v]){
    seg[v] += laz[v] * (high - low + 1);
    if (low != high) {
      laz[2*v + 1] += laz[v];
      laz[2*v+2] += laz[v];
    }
...
void build(int idx, int low, int high, vl& ve){
  if (low == high){
    seg[idx] = ve[low];
    return;
  }
  ...
  int mid = (high + low)/2;
  build(2*idx + 1, low, mid, ve);
  build(2*idx + 2, mid + 1, high, ve);
  seg[idx] = seg[2*idx + 1] + seg[2*idx + 2];
}

ll int range(int idx, int low, int high, int l, int r){
  push(idx, low, high);
  if (low >= l && high <= r) return seg[idx];
  if (low > r || high < l) return 0;
  int mid = (high + low)/2;
  ll int left = range(2*idx + 1, low, mid, l, r);
  ll int right = range(2*idx + 2, mid + 1, high, l, r);
  return left + right;
}

void rupdate(int idx, int low, int high, int l, int r, int val){
  push(idx, low, high);
  ...
  int mid = (high + low)/2;
  rupdate(2*idx + 1, low, mid, l, r, val);
  rupdate(2*idx + 2, mid+1, high, l, r, val);
  seg[idx] = seg[2*idx + 1] + seg[2*idx + 2];
}
...
void solve(){
  ...
  forn(0, m, query){
    ...
    if (o == 1){
      int a, b, u;
      cin >> a >> b >> u;
      rupdate(0, 0, n-1, a-1, b-1, u);
    }
    else{
      int k;
      cin >> k;
      print(range(0, 0, n-1, k-1, k-1));
    }
